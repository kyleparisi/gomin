...
func PostHandler(db *sql.DB, session *sessions.Session) func(_ urlpath.Match, body io.Reader) framework.Response {
	return func(_ urlpath.Match, body io.Reader) framework.Response {
		...
		err = db.QueryRow("SELECT id, `name`, email, password FROM `app_user` where email = ?", strings.ToLower(login.Email)).Scan(&appUser.Id, &appUser.Name, &appUser.Email, &appUser.Password)
		if err != nil {
			panic(err.Error())
		}
